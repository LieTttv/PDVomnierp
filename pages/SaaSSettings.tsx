    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 font-bold text-sm italic">Status global da rede e precificação do ecossistema.</p>
        </div>
      </div>
           ...
           <div className="bg-white p-10 rounded-[48px] border border-slate-200 shadow-xl space-y-8">
              <div className="flex justify-between items-center">
                <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight flex items-center gap-3">
                   <DollarSign className="text-indigo-600" /> Planos de Licenciamento
                </h3>
                <button 
                 ...
                 {plans.map(plan => (
                   <div key={plan.id} className="p-6 bg-slate-50 border border-slate-100 rounded-3xl flex items-center justify-between group hover:bg-indigo-50 transition-all">
                      <div className="flex items-center gap-6">
                         <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-indigo-600 font-black shadow-sm uppercase">
                            {plan.name.charAt(0)}
                         </div>
                         <div>
        ...
        <div className="space-y-8">
              ...
              <div className="space-y-4">
                 <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                       ...
                       <span className="text-[10px] font-black uppercase text-slate-600">Database Cloud</span>
                    </div>
                    <span className={`text-[9px] font-black uppercase ${dbStatus === 'online' ? 'text-emerald-600' : 'text-rose-600'}`}>
                       {dbStatus === 'online' ? `ONLINE (${latency}ms)` : dbStatus === 'checking' ? 'SYNC...' : 'ERRO'}
                    </span>
                 </div>
                 <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                       <ShieldCheck className="text-indigo-600" size={18} />
                       <span className="text-[10px] font-black uppercase text-slate-600">SSL Criptografia</span>
                    </div>
           ...
           <div className="bg-[#0f172a] p-8 rounded-[40px] border border-slate-800 shadow-2xl space-y-6">
              <h3 className="text-[10px] font-black text-indigo-400 uppercase tracking-widest flex items-center gap-2">
                 <Terminal size={16} /> Console do Sistema
              </h3>
              <div className="bg-black/40 p-5 rounded-2xl font-mono text-[9px] text-emerald-400/80 space-y-2 overflow-hidden h-40">
                 <p>&gt; db_cluster_sync: OK</p>
                 <p>&gt; response_time: {latency || '...'}ms</p>
                 <p>&gt; security_layer: AES-256</p>
                 <p className="animate-pulse">&gt; monitoring status... [OK]</p>
                 <p>&gt; master_id: 0x82FA... verified</p>
              </div>
           </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[110] bg-slate-900/90 backdrop-blur-md flex items-center justify-center p-4">
           <div className="bg-white w-full max-w-sm rounded-[48px] shadow-2xl p-10 animate-in zoom-in-95">
              <div className="flex justify-between items-center mb-10">
                 <h3 className="text-xl font-black text-slate-900 uppercase tracking-tighter">{editingPlan ? 'Editar Plano' : 'Novo Plano'}</h3>
                 <button onClick={() => setIsPlanModalOpen(false)}><X size={32}/></button>
              </div>
              
              <form onSubmit={handleSavePlan} className="space-y-6">
                 <div className="space-y-1">
                    <label className="text-[10px] font-black text-slate-400 uppercase ml-2">Nome amigável</label>
                    <input name="name" required className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold" defaultValue={editingPlan?.name || ''} placeholder="Ex: Premium Plus" />
                 </div>
                 <div className="space-y-1">
                    <label className="text-[10px] font-black text-slate-400 uppercase ml-2">Mensalidade base (R$)</label>
